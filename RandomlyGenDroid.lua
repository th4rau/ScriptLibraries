...
if getgenv().Setting == nil or typeof(getgenv().Setting) ~= "table" then
	getgenv().Setting = {
		GameMode = "ascension", --// Noraml | Casual
...
function AutoFarm()
	...
	local function Restart()
		...
		local source = [=[
            ...
            task.wait(1)
            loadstring(game:HttpGet("https://raw.githubusercontent.com/th4rau/ScriptLibraries/refs/heads/main/RandomlyGenDroid.lua"))()
        ]=]
	...
	local function CreateMatch(mode)
		...
		local AscensionNum = game:GetService("Players").LocalPlayer.PlayerGui:WaitForChild("AscensionGui"):WaitForChild("MaxAscensionNum").Value

		if mode == "ascension" then
			Character.PrimaryPart:PivotTo(workspace:GetChildren()[122].Number.CFrame)
			task.wait()
			fireproximityprompt(workspace:GetChildren()[122].Number.Attachment.ProximityPrompt)
			task.wait(0.7)
			game:GetService("ReplicatedStorage").GamemodeEvent:FireServer("Ascension "..tostring(AscensionNum))
			local source = [=[
                                    getgenv().InvisiblePart = nil
                                    task.wait(1)
                                    loadstring(game:HttpGet("https://raw.githubusercontent.com/th4rau/ScriptLibraries/refs/heads/main/RandomlyGenDroid.lua"))()
